...
def order_dog_food(small: int, medium: int, large: int, leftover: float) -> float:
    if not isinstance(small,int) or not isinstance(medium,int) or not isinstance(large,int):
        raise Exception("Please enter whole numbers for each dog size category")

    if not isinstance(leftover, (int, float)):
        raise Exception("Please enter a number for the amount of food in lbs leftover from the previous month.")
    ...
    if sum_dogs > max_capacity :
        raise Exception(
            "You've entered %s which is more dogs than the shelter's max capacity of %d. Please check your numbers and try again." 
            %(sum_dogs, max_capacity)
            )
